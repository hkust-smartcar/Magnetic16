...
void pSmartCar::speedControl(void)
{
	...
	pResource::m_instance->updateSmoothSpeedIncrement(pResource::m_instance->m_pidOutputVal[Type::Speed]);
}
...
void pSmartCar::updateState(void)
{
	if (m_state[StatePos::prev].timeStamp)
	{
		...
		m_state[StatePos::cur].a = (m_state[StatePos::cur].dX - m_state[StatePos::prev].dX)/(System::Time() - m_state[StatePos::prev].timeStamp);
		//		m_state[StatePos::cur].dX = (m_motors[0].getEncoderCount() + m_motors[1].getEncoderCount()) * 0.0523137f / (System::Time() - m_state[StatePos::prev].timeStamp);//m_filter.Filter((m_motors[0].getEncoderCount() + m_motors[1].getEncoderCount()) * 0.5f - pResource::configTable.kCountPerDeg * (m_angle.getAngle() - m_state[StatePos::prev].angle));
		m_state[StatePos::cur].dYaw = /*m_filter[Type::Direction].filter(*/m_angle.getYawOmega()/*)*/;
	}
//	m_state[StatePos::cur].timeStamp = System::Time();


		m_state[StatePos::prev].timeStamp = System::Time();
}

void pSmartCar::updateSmoothSpeedIncrement(const float newSpeed)
{
	//	m_smoothIncrement[IncrementType::SpeedIncrement] = (newAngle - m_idealAngleOffset) * 0.05f;
	m_smoothIncrement[IncrementType::SpeedIncrement] = (newSpeed - m_idealSpeedOffset) * 0.05f;
}

void pSmartCar::updateSmoothSpeedOutput(void)
{
	...
	m_idealSpeedOffset += m_smoothIncrement[IncrementType::SpeedIncrement];
}

float pSmartCar::getSmoothSpeedOutput(void){
	return m_idealSpeedOffset;
}
...
void pSmartCar::updateSpeed(void)
{
	...
	pResource::m_instance->updateSmoothSpeedOutput();
	m_motors[0].setMappedPower(inRange(-5000, m_pidOutputVal[Type::Angle] + tempDirectionOutput + getSmoothSpeedOutput(), 5000));
	m_motors[1].setMappedPower(inRange(-5000, m_pidOutputVal[Type::Angle] - tempDirectionOutput + getSmoothSpeedOutput(), 5000));
}
